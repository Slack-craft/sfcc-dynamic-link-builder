...
Good example of offer that is in SPO inclusions and therefore the Dataset but EXCLUDED from the Catalogue out now page; would be linked via Suspension subcat. Either need a way to handle via the Builder or this would be a Live Link candidate. The issue is that we have the actual PLU list - so we could technically populate Brand and Article type facets. We'd need to expand Category dropdown to include Subcategories as well to drill down. Potential here to also get items not included in the Dataset, if we are linking via a standard Category page (though in this case, it is a straight link of - these three brand facets. So this one is unlikely, but other cases could exist)
